...
// заявки (type=0-расход  type=1-приход)
	...
	function create_zayavka_prihod() {
		...
		// $sql = "INSERT INTO products (progect_id, nazva, kilk, edinica_izm, opus, artikl) VALUES ";
		// $insertArray = array();
		// //foreach ($product as $value) {
		// foreach ($product as $k=>$value) {
		// 	array_push($insertArray,  "(".$progect_id.", '".$value."', ".$kilk[$k].", '".$edinica_izm[$k]."', 'нет', '".$artikl[$k]."')");
		// }
		// $sql .= implode(", ", $insertArray);
		// $this->db->query($sql);
		...
		// $insertArray = array();
		// foreach ($product as $value) {
		// 	array_push($insertArray, "(".$return_z_id.", ".$value.", ".$count[$value].")");
		// }
		// $sql .= implode(", ", $insertArray);
		// $this->db->query($sql);
